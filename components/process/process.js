...
Component({
  ...
  lifetimes: {
    attached: function () {
      // 在组件实例进入页面节点树时执行
      this.setData({
        progressText: (this.properties.currentProgress).toFixed(1) * 10,
  ...
  methods: {
    ...
    buttonMove: function (e) {
      ...
      this.setData({
        ...
        progressText: ((buttonLeft - min) / (max - min) * this.properties.maxProgress).toFixed(1) * 10
        //
      })
    },
    ...
    setCurrentProgress(progress) {
      this.setData({
        ...
        progressText: (progress).toFixed(1) * 10,
